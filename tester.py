...
def test_configuration(algorithm_name, point_filename, sigma, sigma_angles, args):
    ...
    for runnum in range(args.repeats):
        print(f"Running `{algorithm_name}` on `{point_filename}` with sigmas={sigma},{sigma_angles}. Run {runnum+1}/{args.repeats}")
...
if __name__ == "__main__":
    ...
    with open("results.csv", "w") as f:
        ...
        writer = csv.writer(f)
        writer.writerow(["sample", "algorithm", "sigma", "sigma_angle", "RMSE", "time"])

        for fname in filenames:
            for algo in algorithms:
                for sigma in sigmas:
                    for sigma_angle in sigmas_angles:
                        results = test_configuration(algo, fname, sigma, sigma_angle, args)
                        writer.writerow([fname, algo, str(sigma), str(sigma_angle)] + results)
